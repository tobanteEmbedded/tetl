...
static auto test() -> bool
{
    ...
    // "move"
    {
        ...
        CHECK(!mtx.is_locked());
        {
            ...
            CHECK(!l1.owns_lock()); // NOLINT(clang-analyzer-cplusplus.Move)
            CHECK(l2.owns_lock());
            CHECK(mtx.is_locked());

            etl::unique_lock<test_mutex> l3{};
            l3 = etl::move(l2);
            CHECK(!l2.owns_lock()); // NOLINT(clang-analyzer-cplusplus.Move)
            CHECK(l3.owns_lock());
            CHECK(mtx.is_locked());
        }
        CHECK(!mtx.is_locked());
