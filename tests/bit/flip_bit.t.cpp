...
constexpr auto test_all() -> bool
{
    ...
    CHECK(not has_flip_bit<etl::int8_t>);
    CHECK(not has_flip_bit<etl::int16_t>);
    CHECK(not has_flip_bit<etl::int32_t>);
    CHECK(not has_flip_bit<etl::int64_t>);
    CHECK(not has_flip_bit<etl::ptrdiff_t>);

    CHECK(not has_flip_bit<signed char>);
    CHECK(not has_flip_bit<signed short>);
    CHECK(not has_flip_bit<signed int>);
    CHECK(not has_flip_bit<signed long>);
    CHECK(not has_flip_bit<signed long long>);

    CHECK(not has_flip_bit<bool>);
    CHECK(not has_flip_bit<char>);
    CHECK(not has_flip_bit<char8_t>);
    CHECK(not has_flip_bit<char16_t>);
    CHECK(not has_flip_bit<char32_t>);
