...
TEMPLATE_TEST_CASE_SIG("bitset: construct(char const*)", "[bitset]",
    ((size_t N), N), 8, 16, 32, 64)
{
...
TEMPLATE_TEST_CASE_SIG("bitset: to_string", "[bitset]", ((size_t N), N), 8)
{
    ...
    REQUIRE(bits.template to_string<16>() == "00101010"_sv);
    REQUIRE(bits.template to_string<16>('*') == "**1*1*1*"_sv);
    REQUIRE(bits.template to_string<16>('O', 'X') == "OOXOXOXO"_sv);
}