...
[[nodiscard]] static constexpr auto test_linalg_copy_real() -> bool
{
    {
        // 1D static
        auto in_data  = etl::array<T, 4> { T(0), T(1), T(2), T(3) };
        auto out_data = etl::array<T, 4> {};

        auto in  = etl::mdspan<T const, etl::extents<IndexType, 4>> { in_data.data() };
...

    {
        ...
        auto in  = etl::mdspan<T const, etl::dextents<IndexType, 1>> { in_data.data(), 4 };
        auto out = etl::mdspan<T, etl::dextents<IndexType, 1>> { out_data.data(), 4 };
...

    {
        ...
        auto in  = etl::mdspan<T const, etl::extents<IndexType, 2, 2>> { in_data.data() };
...

    {
        ...
        auto in  = etl::mdspan<T const, etl::dextents<IndexType, 2>> { in_data.data(), 2, 2 };
