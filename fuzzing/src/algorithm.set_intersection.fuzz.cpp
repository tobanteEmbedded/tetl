...
[[nodiscard]] static auto fuzz_set_intersection(FuzzedDataProvider& p) -> int
{
    ...
    etl::set_intersection(aview.begin(), aview.end(), bview.begin(), bview.end(), std::back_inserter(eout));
    std::set_intersection(aview.begin(), aview.end(), bview.begin(), bview.end(), std::back_inserter(sout));

    if (eout != sout) {
        std::println(stderr, "set_intersection: '{}' and '{}'", a, b);
