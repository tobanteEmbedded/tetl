...
[[nodiscard]] static auto fuzz_is_permutation(FuzzedDataProvider& p) -> int
{
    ...
    auto const e = etl::is_permutation(va.begin(), va.end(), vb.begin(), vb.end());
    auto const s = std::is_permutation(va.begin(), va.end(), vb.begin(), vb.end());
    if (e != s) {
        return 1;
    }
