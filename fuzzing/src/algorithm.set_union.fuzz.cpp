...
[[nodiscard]] auto fuzz_set_union(FuzzedDataProvider& p) -> int
{
    ...
    etl::set_union(aview.begin(), aview.end(), bview.begin(), bview.end(), std::back_inserter(eout));
    std::set_union(aview.begin(), aview.end(), bview.begin(), bview.end(), std::back_inserter(sout));

    if (eout != sout) {
        std::println(stderr, "set_union: '{}' and '{}'", a, b);
