...
constexpr T incomplete_gamma_inv_decision(const T value, const T a, const T p,
    const T direc, const T lg_val, const int iter_count) noexcept;
...
constexpr T incomplete_gamma_inv_initial_val_1_int_end(
    const T value_inp, const T a) noexcept
{ // internal for a > 1.0
    return max(T(1E-04),
...
constexpr T incomplete_gamma_inv_initial_val_1(
    const T a, const T t_val, const T sgn_term) noexcept
{ // a > 1.0
...
constexpr T incomplete_gamma_inv_initial_val_2(
    const T a, const T p, const T t_val) noexcept
{                       // a <= 1.0
    return (p < t_val ? // if
                pow(p / t_val, T(1) / a)
                      :
...
constexpr T incomplete_gamma_inv_err_val(
    const T value, const T a, const T p) noexcept
{ // err_val = f(x)
...
constexpr T incomplete_gamma_inv_deriv_1(
    const T value, const T a, const T lg_val) noexcept
...
constexpr T incomplete_gamma_inv_recur(const T value, const T a, const T p,
    const T deriv_1, const T lg_val, const int iter_count) noexcept
{
