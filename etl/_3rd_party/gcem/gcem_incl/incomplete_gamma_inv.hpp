...
constexpr auto incomplete_gamma_inv_decision(
    T value, T a, T p, T direc, T lgVal, int iterCount) noexcept -> T;
...
template <typename T>
constexpr auto incomplete_gamma_inv_initial_val_1_int_begin(
    const T tVal) noexcept -> T
{ // internal for a > 1.0
    return (tVal
            - (T(2.515517L) + T(0.802853L) * tVal + T(0.010328L) * tVal * tVal)
                  / (T(1) + T(1.432788L) * tVal + T(0.189269L) * tVal * tVal
                      + T(0.001308L) * tVal * tVal * tVal));
}

template <typename T>
constexpr auto incomplete_gamma_inv_initial_val_1_int_end(
    const T valueInp, const T a) noexcept -> T
{ // internal for a > 1.0
    return max(
        T(1E-04), a * pow(T(1) - T(1) / (9 * a) - valueInp / (3 * sqrt(a)), 3));
}

template <typename T>
constexpr auto incomplete_gamma_inv_initial_val_1(
    const T a, const T tVal, const T sgnTerm) noexcept -> T
{ // a > 1.0
    return incomplete_gamma_inv_initial_val_1_int_end(
        sgnTerm * incomplete_gamma_inv_initial_val_1_int_begin(tVal), a);
}

template <typename T>
constexpr auto incomplete_gamma_inv_initial_val_2(
    const T a, const T p, const T tVal) noexcept -> T
{                      // a <= 1.0
    return (p < tVal ? // if
                pow(p / tVal, T(1) / a)
                     :
...
template <typename T>
constexpr auto incomplete_gamma_inv_initial_val(const T a, const T p) noexcept
    -> T
{
    return (a > T(1) ? // if
                incomplete_gamma_inv_initial_val_1(a,
                    incomplete_gamma_inv_t_val_1(p), p > T(0.5) ? T(-1) : T(1))
                     :
                     // else
...
template <typename T>
constexpr auto incomplete_gamma_inv_err_val(
    const T value, const T a, const T p) noexcept -> T
{ // err_val = f(x)
    return (incomplete_gamma(a, value) - p);
}

template <typename T>
constexpr auto incomplete_gamma_inv_deriv_1(
    const T value, const T a, const T lgVal) noexcept -> T
{ // derivative of the incomplete gamma function w.r.t. x
    return (exp(-value + (a - T(1)) * log(value) - lgVal));
}

template <typename T>
constexpr auto incomplete_gamma_inv_deriv_2(
    const T value, const T a, const T deriv1) noexcept -> T
{ // second derivative of the incomplete gamma function w.r.t. x
    return (deriv1 * ((a - T(1)) / value - T(1)));
}

template <typename T>
constexpr auto incomplete_gamma_inv_ratio_val_1(
    const T value, const T a, const T p, const T deriv1) noexcept -> T
{
    return (incomplete_gamma_inv_err_val(value, a, p) / deriv1);
}

template <typename T>
constexpr auto incomplete_gamma_inv_ratio_val_2(
    const T value, const T a, const T deriv1) noexcept -> T
{
...
template <typename T>
constexpr auto incomplete_gamma_inv_halley(
...
template <typename T>
constexpr auto incomplete_gamma_inv_recur(const T value, const T a, const T p,
    const T deriv1, const T lgVal, const int iterCount) noexcept -> T
{
...
template <typename T>
constexpr auto incomplete_gamma_inv_decision(const T value, const T a,
    const T p, const T direc, const T lgVal, const int iterCount) noexcept -> T
{
    ...
    return (iterCount <= GCEM_INCML_GAMMA_INV_MAX_ITER ? // if
                incomplete_gamma_inv_recur(value - direc, a, p,
                    incomplete_gamma_inv_deriv_1(value, a, lgVal), lgVal,
                    iterCount + 1)
                                                       :
                                                       // else
                value - direc);
...
template <typename T>
constexpr auto incomplete_gamma_inv_begin(
    const T initialVal, const T a, const T p, const T lgVal) noexcept -> T
{
    return incomplete_gamma_inv_recur(initialVal, a, p,
        incomplete_gamma_inv_deriv_1(initialVal, a, lgVal), lgVal, 1);
}
...
constexpr auto incomplete_gamma_inv_type_check(const T1 a, const T2 p) noexcept
    -> TC
{
