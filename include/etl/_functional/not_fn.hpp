...
struct not_fn_t {
    ...
    template <typename... Args>
        requires negate_invocable<F, Args...>
    constexpr auto
    operator()(Args&&... args) && noexcept(noexcept(not etl::invoke(etl::move(f), etl::forward<Args>(args)...)))
        -> decltype(auto)
    {
    ...
    template <typename... Args>
        requires negate_invocable<F const, Args...>
    constexpr auto
    operator()(Args&&... args) const&& noexcept(noexcept(not etl::invoke(etl::move(f), etl::forward<Args>(args)...)))
        -> decltype(auto)
    {
        return not etl::invoke(etl::move(f), etl::forward<Args>(args)...);
    }
