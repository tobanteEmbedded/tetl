...
struct not_fn_t {
    ...
    constexpr auto operator()(Args&&... args) & noexcept(
        noexcept(not etl::invoke(f, etl::forward<Args>(args)...))) -> decltype(auto)
    {
...
struct stateless_not_fn {
    template <typename... Args>
    constexpr auto operator()(Args&&... args) const noexcept(noexcept(!etl::invoke(
        ConstFn, etl::forward<Args>(args)...))) -> decltype(!etl::invoke(ConstFn, etl::forward<Args>(args)...))
    {
