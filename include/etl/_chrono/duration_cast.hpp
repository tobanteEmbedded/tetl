...
struct duration_cast_impl {
    template <typename Rep, typename Period>
    [[nodiscard]] static constexpr auto cast(
    ...
    ) noexcept(is_arithmetic_v<Rep> and is_arithmetic_v<typename ToDuration::rep>) -> ToDuration
    {
        ...
        return ToDuration(
            static_cast<to_rep>(static_cast<CR>(duration.count()) * static_cast<CR>(CF::num) / static_cast<CR>(CF::den))
        );
    }
};
...
[[nodiscard]] constexpr auto duration_cast(
    duration<Rep, Period> const& duration
